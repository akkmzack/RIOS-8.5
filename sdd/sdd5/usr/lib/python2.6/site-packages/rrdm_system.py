...
class System:
    ...
    def __lvm_raid_setup(self,
        ...
        dl = DeviceList(self.spec,
                        lvm_context = (self.disk_array, lvm))
        r=RaidArray()
        raid_sb_ver=self.spec.get_raid_sb_version()
        r.fill_from_system_info (dl,
                                 ...
                                 lvm.get_size_mb(),
                                 raid_sb_ver,
                                 lvm.get_device_list()[0].get_zone(),
	...
	for dev in self.raid_arrays.get_array_list():
        ...
        if do_partition:
            ...
            if hd.get_media() == 'ssd':
                hd.partition_drive(align_boundary = \
                                     self.spec.get_ssd_align_boundary())
            else:
                hd.partition_drive()
