...
from os import remove, popen, stat, path, rename, listdir, uname
...
class HardDisk:
	...
	if kernel_version.startswith('2.6.9'):
	    ...
	    self.name = listdir('/sys/bus/scsi/devices/%s/block' % found_bus)[0]
	    self.size = get_scsi_sysfs_param(found_bus, 'block/%s/size' % self.name)
	...
	self.status="online"
        if self.model.lower() == "virtual disk":
            self.serialnum  = ""
            self.firmware   = ""
            self.__media    = "disk"
        else:
            # If the platform is using hardware RAID, the virtual drive
            # does not have a valid serial and firmware so set it to empty
            # The disk_config.xml file generated will have empty strings
            # but its not used anywhere for HWRAID boxes anyways.
            raid_type = get_raid_type()
            if raid_type == "hardware":
                self.serialnum  = ""
                self.firmware   = ""
                self.__media    = "disk"
            else:
	...
	if self.name == '':
        ...
        part_tbl_type = self.part_tbl.get_part_tbl_type()

        if part_tbl_type == "mbr":
            if align_boundary == NO_ALIGNMENT:
                cmd_line = '/sbin/sfdisk --no-reread -q -uM /dev/%s 2>&1 < %s > /dev/null' % (self.name, name)
            else:
                self.align_partitions(name, align_boundary)
                cmd_line = '/sbin/sfdisk --force --no-reread -q -uS /dev/%s 2>&1 < %s > /dev/null' % (self.name, name)

        else:
            # Parted '--align minimal' flag causes 512 bytes getting consumed from the
            # first partition on SSDs causing unaligned partitions. We are already supplying
            # aligned partitions from specs hence using this flag is of little or no use.
            # Removing it from here and mkpart cmd line
            cmd_line = '/sbin/parted -s /dev/%s mklabel gpt > /dev/null 2>&1' % (self.name)
	    ...
	    if err:
                raise rrdm_error ('Failed to partition device /dev/%s' % self.name)

            err = run_shell_cmd('/sbin/udevadm settle --timeout=20')
            if err:
                rlog_notice('Udev settle failed after partitioning %s' % \
                            self.name)
                # if settling fails, give a 5s grace period and proceed
                sleep(5)

            # align the partitions
            align_sectors = self.part_tbl.get_align_sectors()
            if  align_sectors != '':
                align_cmd = '/sbin/align_partitions.sh /dev/%s %s' % (self.name, align_sectors)
                err = run_shell_cmd(align_cmd)
                if err:
                    raise rrdm_error ('Failed to align partitions on /dev/%s size %s' % (self.name, align_sectors)) 

            # Give a grace period for devices to appear
            # If using parted until this block we've only created GPT label so no need to wait
            if part_tbl_type != "gpt":
                for part in self.part_tbl.get_partition_list():
                    dev = '/dev/%s' % part.get_devname()
                    for i in range(3):
                        if not exists(dev):
                            print "Device %s not present, will wait till its created" % dev
                            sleep(2)
                        else:
                            break
        ...
        if part_tbl_type == "gpt":
            ...
            for i, line in enumerate(f):
                cmd_line = '/sbin/parted -s /dev/%s mkpart > /dev/null 2>&1 %s' % (self.name, line)
                if dry:
                    print cmd_line
                else:
                    err = run_shell_cmd(cmd_line)
                    if err:
                        # This might be, WARNING: the kernel failed to re-read the partition table
                        # Udev is busy creating link for previous partition on same disk while we are
                        # trying to write updated partition table on disk which creates race. In this case
                        # don't give up but give it few seconds, run partprobe and notify kernel about
                        # partition table changes.
                        sleep(2)
                        cmd_line = '/sbin/partprobe /dev/%s' % self.name
                        err = run_shell_cmd(cmd_line)
                        if err:
                            raise rrdm_error ('Failed to update ptable info for /dev/%s' % \
                                                self.get_devname())
                        else:
                            # Now we need to make sure that partprobe did its job and we see
                            # correct partitions created till this point. Bail out if partition
                            # count is incorrect
                            cmd_line = 'grep %s /proc/partitions | wc -l' % self.name
                            output = run_shell_cmd(cmd_line, True)
                            if int(output) - 1 != i + 1:
                                raise rrdm_error ('Failed to partition device /dev/%s' % self.get_devname())

            # After last partition on disk is created udev might still be
            # creating link for it, settle udev
            err = run_shell_cmd('/sbin/udevadm settle --timeout=20')
            if err:
                rlog_notice('Udev settle failed after partitioning %s' % \
                            self.name)
    ...
    ###########################################################################
    # pfile    - Path to a file that contains a partition table suitable for 
    #            input to sfdisk.
    # boundary - A number that signifies the alignment boundary in bytes.
    # 
    # This method writes out the partition table back into pfile after ensuring
    # that the individual partitions are aligned along the specifid boundary.
    # Partition start and size are in units of sectors in the resulting file.
    ###########################################################################
    def align_partitions(self,pfile,boundary):
        # sfdisk takes input in the following format
        # <start> <size> <id> <bootable>
        # The original partition table has lines of the form 
        # ,20,83,*  
        # Note that <start> is not specified, and the numbers are in MB
        ptable = open(pfile, 'r')
        partitions = ptable.read().splitlines()
        ptable.close()
        partitions_sec = []
        ...
        for i in range(len(partitions)):
            partitions_sec.append('')
            # The first partition must start at sector 1.
            # This will ensure that realignment will not affect the
            # physical location of the RVBD super block
            if i == 0:
                start_of_part = 1
            else:
                # The remaining partitions will simply follow their previous.
                start_of_part = start_of_prev_part + size_of_prev_part

            partitions_sec[i] = str(start_of_part) + ','
            this_partition = partitions[i].split(',')
            part_size = int(this_partition[1])
            part_id = this_partition[2]
            part_bootable = this_partition[3]

            # Adjust the size of the partition so that it is divisible by
            # the alignment boundary, if need be.
            num_align_chunks = part_size * MB / boundary
            needs_alignment = part_size * MB % boundary
            if needs_alignment != 0:
                num_align_chunks = num_align_chunks + 1
            part_aligned_size = num_align_chunks * boundary

            # Convert the partition size units from MB to sectors
            part_size_sectors = part_aligned_size/bytes_per_sector

            # Subtract 1 from the size of the first partition because
            # the first partition starts from sector 1 as opposed to 
            # sector 0. Sector 0 has MBR on it.
            if i == 0:
                part_size_sectors = part_size_sectors - 1

            partitions_sec[i] = partitions_sec[i] + str(part_size_sectors) \
                            + ',' + part_id + ',' + part_bootable + '\n'

            # Save the size and start location of the i'th partition, so that
            # we know where to start the (i+1)'th partition. 
            start_of_prev_part = start_of_part
            size_of_prev_part = part_size_sectors

        ptable = open(pfile, 'w')
        # Write out the partition table with lines of the form
        # <start> <size> <id> <bootable>
        # 65536,40960,83,*  
        ptable.write(''.join(partitions_sec))
        ptable.close()
...
class PartitionTable:
    def __init__(self, zone_cfg):
        self.partitions = []
        self.part_tbl_type = zone_cfg.get_layout().get_part_tbl_type()
        self.gpt_part_align = zone_cfg.get_layout().get_gpt_part_align()
        self.align_sectors = zone_cfg.get_align_sectors()
        ...
        for part in part_list:
            ...
            p.make_partition (part.get_part_id(), part.get_part_size(zone_cfg), 
                              ...
                              fill_to_end=part.get_fill_to_end())
    ...
    def get_part_tbl_type(self):
        return self.part_tbl_type

    def get_gpt_part_align(self):
        return self.gpt_part_align
    ...
    # generates output usable by sfdisk/parted to partition a drive
    ...
    def gen_disk_geom(self):
        ...
        for part in self.partitions:
            ...
            if self.part_tbl_type == "mbr":
                if (part.type != '0f'):
                    if first_part:
                        part_str.append(',%d,%s,*' % (size, part.type))
                        first_part = False
                    elif (part.get_fill_to_end() == 'true'):
                ...
                if first_part:
                    if self.get_gpt_part_align() > 0 and self.get_gpt_part_align() != None:
                        # This is a special case where we want to align the partitions
                        # on stripe boundaries defined in the specs. In order to keep 
                        # all the other models using parted untouched, I am adding 
                        # the partition string creation in this if-else block.
                        # Currenlty all sturgeons and Redfins seem to be unaligned, but
                        # we do not care as much in software RAID.
                        prev_total_size = int(self.get_gpt_part_align()) + total_size
                        size += prev_total_size
                        total_size += size
                        part_str.append('%s %dMiB %dMiB' % (part.type, prev_total_size, total_size))
                    else:
                        prev_total_size = total_size
                        total_size += size
                        part_str.append('%s %d %d' % (part.type, prev_total_size, total_size))
                    first_part = False
                else:
                    prev_total_size = total_size
                    total_size += size
                    if self.get_gpt_part_align() > 0 and self.get_gpt_part_align() != '':
                        part_str.append('%s %dMiB %dMiB' % (part.type, prev_total_size, total_size))
                    else:
                        part_str.append('%s %d %d' % (part.type, prev_total_size, total_size))
