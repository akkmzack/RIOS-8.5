...
/* wbcVampireParams->falgs */
#define WBC_VAMPIRE_PARAM_FLAGS_NTLM		0x00000001
#define WBC_VAMPIRE_PARAM_FLAGS_KRB5		0x00000002


struct wbcVampireParams {
    const char *username;
    const char *userrealm;
    const char *userpassword;
    const char *objectname;
    const char *objectrealm; 
    const char *objectdn; 
    const char *objectspn; 
    uint32_t flags;
};

enum wbcVampireStatus {
        WBC_VAMPIRE_OK, 
        WBC_VAMPIRE_ERROR, 
        WBC_VAMPIRE_ILLEGAL_ARGUMENT, 
        WBC_VAMPIRE_UNKNOWN_DOMAIN, 
        WBC_VAMPIRE_NO_LDAP_SERVER, 
        WBC_VAMPIRE_UNKNOWN_OBJECT, 
        WBC_VAMPIRE_LOGON_FAILED, 
        WBC_VAMPIRE_ACCESS_DENIED, 
        WBC_VAMPIRE_DRSUAPI_CONNECT_FAILED, 
};

struct wbcVampireInfo {
    uint32_t count; /* No of keys */
    struct wbcVampireKeys {
        uint32_t len;
        uint32_t enctype;
        uint32_t kvno;
        char *key;
    ...
    char* objectdn;
    struct wbcVampireInfoStatus {
        enum wbcVampireStatus vampire_status;
        uint32_t nt_status;
    } status;
    char* result_message;
};

wbcErr wbcVampire(const struct wbcVampireParams *params, struct wbcVampireInfo **info);
