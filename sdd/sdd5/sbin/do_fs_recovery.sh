...
do_3ware_var_check()
{
    ...
    if [ $? -ne 0 -o $part -eq 1 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            if [[ ${FSTYPE} -eq 0 ]]; then
                /sbin/mke2fs -qF -O ^resize_inode -L VAR -t ext4 /dev/sda2
            else
...
do_sw_var_check()
{
	...
	if [ ${FORCEIT} -eq 1 ]; then
        ...
        if [ $? -eq 0 -a ${FORCEIT} -eq 0 ]; then
            ...
            if [[ ${FSTYPE} -eq 0 ]]; then
                /sbin/mke2fs -qF -O ^resize_inode -L VAR -t ext4 ${RDEV} ${blocks}
            else
...
do_fs_recovery_common()
{
    ...
    if [ $? -ne 0 ]; then
        ...
        # Do Whitewater specific recovery and exit
        if [ "$HAVE_DO_FS_RECOVERY_GRAFT_1" = "y" ]; then
            do_fs_recovery_graft_1
        fi
        ...
        if [[ ${FSTYPE} -eq 0 ]]; then
            /sbin/mke2fs -qF -O ^resize_inode -t ext4 ${PART_DEV} ${blocks}
        else
...
###############################################################################
# Get the VAR filesystem type.
# Used to support ext4 FS on VAR.
# Return 0 if FS is ext4, 1 if not
###############################################################################
is_fstype_ext4()
{
    FSTYPE=$( $RRDM_TOOL_PY -l | grep "var" | tr ":" " " | awk '{print $3}' | grep "ext4" )
    if [ $? -eq 0 ]; then
        echo "0"
    else
        echo "1"
...
case "x${VOL_NAME}" in
	...
	exit 0
    ;;
    # couple of other miscellaneous filesystem with no recovery
    # procedure, but not an error
    #
    "xshark")
	exit 0
    ;;
