...
#
#  URL:       $URL: svn://svn.nbttech.com/mgmt-fwk/branches/kauai_373_fix_branch/framework/src/base_os/common/script_files/writeimage.sh $
#  Revision:  $Revision: 105490 $
#  Date:      $Date: 2013-05-22 18:28:04 -0700 (Wed, 22 May 2013) $
...
# File used to control shadow write caching
SHADOW_WCE_FILE="/sys/block/sha/shadow/wce"

usage()
{
    echo "usage: $0 -m [-M MODEL] [-u URL] [-f FILE] [-L LAYOUT_TYPE] -d /DEV/N1"
    echo "             [-p PARTNAME -s SIZE] [-t] [-k KERNEL_TYPE]"
    echo "             [-S STORAGE PROFILE] [-A ALIGNMENT_BOUNDARY] "
    echo "usage: $0 -i [-M MODEL] [-u URL] [-f FILE] [-d /DEV/N1] -l {1,2} [-t]"
    echo "             [-k KERNEL_TYPE] [-A ALIGNMENT_BOUNDARY] "
...
# =================================================
# Function to extract partial image contents
# =================================================
do_extract_partial_image()
{
    local tar_file="$1"
    local opts="$2"
    ...
    if [ -f "$SHADOW_WCE_FILE" ]; then
	echo "--- disabling write cache on shadow driver"
	old_wce_val=$(cat "$SHADOW_WCE_FILE")
	echo "0" >"$SHADOW_WCE_FILE"
    fi

    if [ "${image_ext}" = "tbz" ]; then
	z_flag="-j"
    elif [ "${image_ext}" = "tgz" ]; then
	z_flag="-z"
    elif [ "${image_ext}" = "tar" ]; then
	z_flag=""
    else
        echo "*** Invalid extension on image inner file: ${image_ext}"
        cleanup_exit 17 #lc_err_upgrade_image_integrity_failure
    fi

    FAILURE=0
    dd bs=1M ${DD_IFLAG} if=${tar_file} | 
	tar -f - ${z_flag} -x${TAR_VERBOSE} ${opts} 2>&1 || FAILURE=1
    
    maybe_dropcache

    if [ -f "$SHADOW_WCE_FILE" ]; then
	echo "--- resetting shadow driver write cache to original value: $old_wce_val"
	# Sync to ensure all dirty data has completed writeback before 
	# we re-enable wce
	sync
	echo "$old_wce_val" >"$SHADOW_WCE_FILE" 
    fi

    if [ ${FAILURE} -eq 1 ]; then
        echo "*** Could not extract files from ${SYSIMAGE_FILE_IMAGE}"
        cleanup_exit 17 #lc_err_upgrade_image_integrity_failure
    fi
}
...
do_partition_disks()
{
    ...
    eval 'sector_align_size="${IL_LO_'${IL_LAYOUT}'_ALIGN_DISK_SIZE}"'
	...
	temp_ptable=
        ...
        if [ "x${PARTITION_TYPE}" = "x-uS" ]; then
            # Create and partition drive by sectors.  This is recommended
            # for new 4k-sector HDDs, as this is more precise and would not
            # cause mis-alignment.

            BYTES_PER_SECTOR=512
            SKIP_SECTORS=`expr ${ALIGNMENT_BOUNDARY} / ${BYTES_PER_SECTOR}`

            start_sec=0
            add_sec=${SKIP_SECTORS}
            ...
            for pn in ${part_num_list}; do
                ...
                found=0
                for part in ${part_list}; do
    		# prepare this for FLASHRRDM_RAID layout
    		eval 'raid_parts="${raid_parts} ${IL_LO_'${IL_LAYOUT}'_PART_'${part}'_RAIDED_PART_NUM}"'
                    ...
                    # Calculate num_sec and start_sec, both required in
                    # partition by sectors.
                    # start_sec is ensured to lie at a 4MB boundary.
                    id=${part_id}
                    num_meg=${curr_size}
                    num_sec=`expr ${num_meg} \* 2048`
                    start_sec=`expr ${start_sec} + ${add_sec}`
                    start_sec=`expr ${start_sec} / ${SKIP_SECTORS}`
                    start_sec=`expr ${start_sec} \* ${SKIP_SECTORS}`
                    ...
                    printf "${start_sec},${num_sec},${id},${bootable}\n" >> ${temp_ptable}

                    # Calculate the next partition start offset.
                    add_sec=${SKIP_SECTORS}
                    if [ "x${num_sec}" != "x" ]; then
                        add_sec=`expr ${num_sec} + ${SKIP_SECTORS}`
                    fi
            ...
            # Create and partition drive by megabytes.  This is the
            # original way of partitioning, but would not work well
            # with new 4k-sector HDDs.  There will most likely be
            # mis-alignment warnings using partition by megabytes on
            # 4k-sector HDDs.
	...
	if [ ${PTOOL} = "parted" -a ${target} = "DISK2" ]; then
	    ...
	    FAILURE=0
	    cat ${temp_ptable} | sfdisk -L ${PARTITION_TYPE} --no-reread ${target_dev} || FAILURE=1
	    if [ ${FAILURE} -eq 1 ]; then
		echo "*** Could not make partition table on ${target_dev}"
		cleanup_exit 12 #lc_err_upgrade_partition_failure
	    fi
        ...
        if [ "x$IL_LAYOUT" == "xFLASHRRDM_RAID" ]; then

            echo "=== Zeroing existing partition table on $USER_TARGET_DEV_2"
	    FAILURE=0
            dd if=/dev/zero of=${USER_TARGET_DEV_2} bs=512 count=1 || FAILURE=1
            if [ ${FAILURE} -eq 1 ]; then
		echo "*** Could not zero out ${USER_TARGET_DEV_2}"
		cleanup_exit 12 #lc_err_upgrade_partition_failure
            fi

	    echo "=== Writing partition table to $USER_TARGET_DEV_2"
	    FAILURE=0
	    cat ${temp_ptable} | sfdisk -L ${PARTITION_TYPE} --no-reread ${USER_TARGET_DEV_2} || FAILURE=1
	    if [ ${FAILURE} -eq 1 ]; then
		# USER_TARGET_DEV_2 is defined as MODEL_BOOTDEV_2 in model_xxx
		echo "*** Could not make partition table on $USER_TARGET_DEV_2"
		cleanup_exit 12 #lc_err_upgrade_partition_failure
	    fi

	    raid_parts=`echo "${raid_parts}" | \
		tr ' ' '\n' | sort -n | uniq | tr '\n' ' '`

	    eval 'raid_ndisk="${IL_LO_'${IL_LAYOUT}'_TARGET_NUM_TARGETS}"'
	    eval 'raid_level="${IL_LO_'${IL_LAYOUT}'_TARGET_RAID_LEVEL}"'
	    eval 'raid_mdata="${IL_LO_'${IL_LAYOUT}'_TARGET_RAID_METADATA}"'

	    echo "=== Creating raid for $USER_TARGET_DEV_1 & $USER_TARGET_DEV_2"
 	    for raidp in ${raid_parts}; do
		eval 'cmd="/mfg/mdadm --create '${IL_LO_FLASHRRDM_RAID_DEVNAME_PREFIX}''${raidp}' --level='${raid_level}' --raid-disks='${raid_ndisk}' '${USER_TARGET_DEV_1}'p'${raidp}' '${USER_TARGET_DEV_2}'p'${raidp}' --metadata='${raid_mdata}'"'

		sleep 10
		# todo: check error from create
		`yes | $cmd`
	    done
	fi

        if [ -n "${sector_align_size}" ]; then
            /sbin/align_partitions.sh "${target_dev}" "${sector_align_size}"
        fi
    done
}
...
do_raid_fixup() 
{
	...
	for pair in ${rrdm_res}; do
		...
		fi_ext_excl=""
                ...
                fi_mount="/data"
		fi_ext_excl=""
		if [ "${BUILD_PROD_PRODUCT}" == "RBT_CB" ]; then
		    fi_options="barrier=1,nodiratime"
		    if [ x$fstype = 'xext4' ]; then
		        fi_options="${fi_options},user_xattr,journal_checksum,delalloc"

                        # For Whitewater 730/2030/3030, we need allocate more
                        # inodes to support archival backup.
                        local CB_MODEL=$(sg_map -i -x | grep "RIVERBED" | grep "ATX" | awk '{print $8}' | cut -d "-" -f 2)
                        case ${CB_MODEL} in
                            "730"|"2030"|"3030")
                                inode_option="10000"
                                ;;
		...
		fi_ext_excl=""
                ...
                fi_ext_excl=""
                if [ "${BUILD_PROD_PRODUCT}" == "RBT_CB" ]; then
                    fi_options="noatime,nodiratime"
                fi
            elif [ "x$name" = "xvarbackup" ]; then
                fi_label="VARBACKUP"
                fi_dir="/"
                fi_ext="./"
                fi_ext_pre=$fi_ext
	    ...
	    eval 'IL_LO_'${IL_LAYOUT}'_PART_'$fi_label'_FSTYPE='$fstype
            if [ "${BUILD_PROD_PRODUCT}" == "RBT_CB" ]; then
                eval 'IL_LO_'${IL_LAYOUT}'_PART_'$fi_label'_OPTIONS='$fi_options
                eval 'IL_LO_'${IL_LAYOUT}'_PART_'$fi_label'_INODES='$inode_option
            fi
...
do_make_filesystems()
{
    ...
    for target in ${TARGET_CHANGE_LIST}; do
        ...
        for part in ${change_parts}; do
            ##echo "part= ${part}"
            eval 'part_id="${IL_LO_'${IL_LAYOUT}'_PART_'${part}'_PART_ID}"'
	    eval 'part_num="${IL_LO_'${IL_LAYOUT}'_PART_'${part}'_RAIDED_PART_NUM}"'

            if [ "x$IL_LAYOUT" == "xFLASHRRDM_RAID" ] && [ "x$part_num" != "x" ]; then
		eval 'dev="'${IL_LO_FLASHRRDM_RAID_DEVNAME_PREFIX}''${part_num}'"'
	    else		
		eval 'dev="${IL_LO_'${IL_LAYOUT}'_PART_'${part}'_DEV}"'
            ...
            eval 'inodes="${IL_LO_'${IL_LAYOUT}'_PART_'${part}'_INODES}"'
		...
		lpct=0
			...
			cleanup_exit 13 #lc_err_upgrade_fs_creation_failure
                ...
                FAILURE=0
                if [ "$fstype" = "ext4" ]; then
                    MKE2FS_COMMAND="mke2fs -t ext4 ${MKE2FS_BASE_OPTIONS},64bit ${inode_args} -q ${label_args} ${jargs} ${dev} ${blocks}"
                else 
                    MKE2FS_COMMAND="mke2fs ${MKE2FS_BASE_OPTIONS} -q ${label_args} ${jargs} ${dev} ${blocks}"
                fi
                case ${IL_LAYOUT} in
                    "BOBRDM"|"BOBSTD")
...
do_check_storage_profile()
{
    ...
    do_extract_partial_image "${SYSIMAGE_FILE_IMAGE}" \
		"-C ${EXTRACT_DIR} ./opt/hal/lib/specs"
...
#     Uses SYSIMAGE_USE_URL, SYSIMAGE_URL, and ORIG_SYSIMAGE_FILE
# parameter $1 is the backup directory where the image should be copied
# ==================================================
do_backup_system_image()
{
    FAILURE=0
    target_dir=$1
    if [ ! -d ${target_dir} ]; then
        mkdir -p ${target_dir} || FAILURE=1
        if [ ${FAILURE} -eq 1 ]; then
            echo "*** Could not create directory ${target_dir} to save images"
            cleanup_exit 16 #lc_err_upgrade_image_failure
        else
            echo "==== Created directory ${target_dir} to save images"
        fi
    else
        echo "==== Found directory ${target_dir} to save images"
    fi
    SOURCE_IMAGE=
    LOCAL_COPY=${target_dir}/1.img

    if [ $SYSIMAGE_USE_URL -eq 1 ]; then
        SOURCE_IMAGE=${SYSIMAGE_URL}
        /usr/bin/wget -O ${LOCAL_COPY} ${SOURCE_IMAGE} || FAILURE=1
    else
        SOURCE_IMAGE=${ORIG_SYSIMAGE_FILE}
        cp -p ${SOURCE_IMAGE} ${LOCAL_COPY} || FAILURE=1
    fi

    if [ ${FAILURE} -eq 1 ]; then
        echo "*** Could not backup system image file ${SOURCE_IMAGE}"
        cleanup_exit 16 #lc_err_upgrade_image_failure
    else
        echo "==== Copied system image file ${SOURCE_IMAGE} to ${LOCAL_COPY}"
    fi
}
...
PARSE=`/usr/bin/getopt -- 'miu:f:k:M:d:l:p:P:s:L:rtVS:z:q:A:' "$@"`
...
while true ; do
	...
	-M) MODEL=$2 shift 2 ;;
        ...
        -A) PARTITION_TYPE=-uS; ALIGNMENT_BOUNDARY=$2; shift 2;;
        --) shift ; break ;;
...
# rootfs used during manufacture time doesnt
# allow O_DIRECT to be used, so only use direct dd
# option for upgrades.
if [ ${DO_MANUFACTURE} -eq 1 ] ; then
    DD_IFLAG=
    DD_OFLAG=
...
# Stop MD arrays for FLASHRRDM_RAID layout on a manufacture to remove remnants of a previous installation.
if [ "x$IL_LAYOUT" == "xFLASHRRDM_RAID" -a ${DO_MANUFACTURE} -eq 1 ]; then
    echo "==== Stopping all /dev/md* for FLASHRRDM_RAID layout"
    /mfg/mdadm -S /dev/md*
fi
...
# Number of failure encounter when checking the disk size. In the raid case,
# there is another device to check if the first one fails.
fail_count=0
...
for tn in ${TARGET_NAMES}; do
    ...
    if [ ${FAILURE} -eq 1 -o -z "${imdev_size_k}"  ]; then
        echo "*** Target ${imdev}: could not determine size for ${imdev}"
        if [ "x$IL_LAYOUT" == "xFLASHRRDM_RAID" ]; then
            echo "    Continue with next target disk."
...
if [ "x$IL_LAYOUT" == "xFLASHRRDM_RAID" ]; then
    if [ ${fail_count} -gt 1 ]; then
        echo "*** Target: could not determine size of the install device"	
        cleanup_exit 14 #lc_err_upgrade_invalid_layout
    fi
fi
...
if [ ${USE_TMPFS} -eq 1 ]; then
	...
	if [ ${NO_PARTITIONING} -eq 0 -a ${NO_REWRITE_FILESYSTEMS} -eq 0 ]; then
        ...
        if [ "${curr_fstype}" = "ext4" ]; then
            mount -t ext4 ${curr_dev} ${mount_point} || FAILURE=1
        else
...
# Check sha512sums, md5sums files in that order.
# Note that md5 check will fail in FIPS mode.  The USE_FIPS_MODE
# environment variable is used because calling writeimage.sh with
# OPENSSL_FORCE_FIPS_MODE=1 will set it for mddbreq and cause it
# to fail using DES in the process of reading the manufacturing db.
check_sums() {
    local retval=1
    local alg=''
    for alg in sha512 md5 ; do
        if [ -x /usr/bin/${alg}sum -a -f ${alg}sums ]; then
            if [ "x$USE_FIPS_MODE" != x ]; then
                OPENSSL_FORCE_FIPS_MODE=1 /usr/bin/${alg}sum -c ${alg}sums > /dev/null && retval=0
            else
                /usr/bin/${alg}sum -c ${alg}sums > /dev/null && retval=0
            fi
            break
        fi
    done
...
if [ "${FILE_TYPE}" = "img" ]; then
    ...
    FAILURE=0
    /usr/bin/unzip -q ${SYSIMAGE_FILE} -d ${EXTRACT_DIR} || FAILURE=1
    if [ ${FAILURE} -eq 1 ]; then
    ...
    FAILURE=0
    (cd ${EXTRACT_DIR} ; check_sums ) || FAILURE=1
    if [ ${FAILURE} -eq 1 ]; then
	...
	*)
        ...
        if [ "x${IMAGE_BUILD_CB_SV_ON_VAR_SUPPORT}" = "x" ]; then
            # CB model node is different to the flex node
            # Use a different variable and don't overwrite the MODEL var
            # SH builds will have the IMAGE_BUILD_8150_SV_ON_VAR_SUPPORT set/unset
            # as well and we will overwrite the flex model here unintentionally
            MODEL_CB=`${MDDBREQ} -v ${MFDB} query get - /rbt/mfd/model`
            case ${MODEL_CB} in
                "510"|"710"|"2010")
                    echo "*** Cannot upgrade to version as your secure vault may be corrupted with this operation"
                    cleanup_exit 20 #lc_err_sv_moved_back_to_var
...
# Remove the exracted image as we are using up tmpfs spacewith duplicates
rm -f ${SYSIMAGE_FILE}
...
echo "==== Extracting image contents from: ${SYSIMAGE_FILE_IMAGE}"
...
for target in ${TARGET_CHANGE_LIST}; do
    ...
    for location in ${loc_list}; do
        ...
        eval 'curr_part="${IL_LO_'${IL_LAYOUT}'_LOC_'${location}'_PART}"'
	eval 'curr_part_num="${IL_LO_'${IL_LAYOUT}'_PART_'${curr_part}'_RAIDED_PART_NUM}"'
	eval 'curr_dev="${IL_LO_'${IL_LAYOUT}'_PART_'${curr_part}'_DEV}"'
        eval 'curr_target="${IL_LO_'${IL_LAYOUT}'_PART_'${curr_part}'_TARGET}"'
        ...
        if [ "x$IL_LAYOUT" == "xFLASHRRDM_RAID" ] && [ "x$curr_part_num" != "x" ]; then
	    eval 'curr_dev="'${IL_LO_FLASHRRDM_RAID_DEVNAME_PREFIX}''${curr_part_num}'"'
	    eval 'curr_target_dev="'${IL_LO_FLASHRRDM_RAID_DEVNAME_PREFIX}''${curr_part_num}'"'
	fi
        ...
        if [ "${curr_part}" = "META" -o "${curr_part}" = "LOGS" -o \
             -z "${curr_fstype}" -o "${curr_fstype}" = "swap" ]; then
            echo "-- Nothing to do for this location."
            continue
        fi

        if [ "x$IL_LAYOUT" == "xFLASHRRDM_RAID" ] && [ "x$curr_part_num" != "x" ]; then
            mount_point="${TMP_MNT_IMAGE}/${curr_target}${IL_LO_FLASHRRDM_RAID_DEVNAME_PREFIX}${curr_part_num}${curr_extract_prefix}"
            chdir_to="${TMP_MNT_IMAGE}/${curr_target}${IL_LO_FLASHRRDM_RAID_DEVNAME_PREFIX}${curr_part_num}"
	else		
            mount_point="${TMP_MNT_IMAGE}/${curr_target}/${curr_part}/${curr_extract_prefix}"
	    ...
	    #XXX/evan
            BOOTERMGR_1|BOOTMGR_1|BOOTMGR_2|REPL_BOOTMGR_1|CFG) 
                ...
                echo "-- Installing grub on ${curr_target_dev}"
		...
		if [ "x$IL_LAYOUT" == "xFLASHRRDM_RAID" ]; then
		    dev1=$(basename $USER_TARGET_DEV_1)
		    dev2=$(basename $USER_TARGET_DEV_2)
		    hd0=`hwtool.py -q disk=map | grep $dev1 | awk '{print $3}'`
		    hd1=`hwtool.py -q disk=map | grep $dev2 | awk '{print $3}'`
		    ( echo -e "device (hd0) /dev/$hd0\n" ; \
		      echo -e "root (hd0,0)\n" ; \
		      echo -e "setup (hd0)\n" ; \ 
		      echo -e "device (hd1) /dev/$hd1\n" ; \
		      echo -e "root (hd1,0)\n" ; \
		      echo -e "setup (hd1)\n" ; \
		      echo -e "quit\n" ; \ ) | grub
		fi
	...
	if [ "${location}" = "CFG" -o "${location}" = "IMG1" -o "${location}" = "IMG2" ]; then 
        ...
        do_extract_partial_image "${SYSIMAGE_FILE_IMAGE}" \
		"-C ${chdir_to} ${exclude_arg} ${curr_extract}"
		...
		if [ $MOD_100 ]; then
            ...
            VAR_1)
                echo "-- Post-extraction work for: ${location}"
                if [ ${DO_MANUFACTURE} -eq 1 ]; then
                    images_dir="/var/opt/tms/.saved_images"
                    echo "---- Copy system image to ${chdir_to}${images_dir}"
                    do_backup_system_image "${chdir_to}${images_dir}"
                else
                    echo "---- Nothing to do for this location."
                fi
                ;;
...
if [ "x${IL_LAYOUT}" != "xHDRRDM" ]; then
    if [ ${DO_MANUFACTURE} -eq 1 ]; then
        if [ "x$IL_LAYOUT" == "xFLASHRRDM_RAID" ]; then
            /sbin/aigen.py -m -d ${IL_LO_FLASHRRDM_RAID_DEVNAME_PREFIX} -l 1
	else
	...
	if [ ${INSTALL_IMAGE_ID} != 1 -a ${INSTALL_IMAGE_ID} != 2 ]; then
	    INSTALL_IMAGE_ID=-1
	fi
