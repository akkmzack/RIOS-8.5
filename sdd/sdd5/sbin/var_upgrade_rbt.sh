#
#  Filename:  $Source$
#  Revision:  $Revision: 121544 $
#  Date:      $Date: 2013-02-15 21:04:57 -0800 (Fri, 15 Feb 2013) $
#  Author:    $Author: dchu $
# 
...
case "$versions" in
    ...
    #--------------------------------------------------------------#
    #                                                              #
    #  Bob/Hope  Lanai/8.0 upgrades                                #
    #                                                              #
    #--------------------------------------------------------------#
    558_559)
        MFG_TYPE=`/opt/tms/bin/hald_model -m`
        VSP_VMLOCAL_DHCPD_LEASE="/opt/vmware/vmware_workstation/etc/vmware/networking/vmlocal/dhcpd/dhcpd.leases"
        VSP_VAR_VMLOCAL_DHCPD_LEASE_DIR="/var/etc/vmware/vmlocal/dhcpd"
        # Create the vmware workstation vmlocal dhcpd leases directory
        if [ "x$MFG_TYPE" = "xrvbd_ex" ]; then
            mkdir -p $VSP_VAR_VMLOCAL_DHCPD_LEASE_DIR
            if [ -f $VSP_VMLOCAL_DHCPD_LEASE ]; then
                cp $VSP_VMLOCAL_DHCPD_LEASE $VSP_VAR_VMLOCAL_DHCPD_LEASE_DIR
            fi
        fi
        ;;

    559_560)
        # Remove deprecated disk load stats bug 111480
        rm -f /var/opt/tms/stats/sample-336.dat
        rm -f /var/opt/tms/stats/chd-337.dat
        rm -f /var/opt/tms/stats/chd-338.dat
        rm -f /var/opt/tms/stats/chd-339.dat
        rm -f /var/opt/tms/stats/chd-340.dat
        ;;

    560_561)
        # Remove deprecated SDR stats as per Bug 118416 
        rm -f /var/opt/tms/stats/sample-384.dat
        rm -f /var/opt/tms/stats/chd-385.dat
        rm -f /var/opt/tms/stats/chd-386.dat
        rm -f /var/opt/tms/stats/chd-387.dat
        rm -f /var/opt/tms/stats/chd-388.dat
        ;;

    561_562)
        # Update the paths for VIL
        MFG_TYPE=`/opt/tms/bin/hald_model -m`
        VSP_VMLOCAL_DHCPD_LEASE="/opt/vmware/vmware_vil/etc/vmware/networking/vmlocal/dhcpd/dhcpd.leases"
        VSP_VAR_VMLOCAL_DHCPD_LEASE_DIR="/var/etc/vmware/vmlocal/dhcpd"
        # Create the vmware VIL vmlocal dhcpd leases directory
        if [ "x$MFG_TYPE" = "xrvbd_ex" ]; then
            mkdir -p $VSP_VAR_VMLOCAL_DHCPD_LEASE_DIR
            if [ -f $VSP_VMLOCAL_DHCPD_LEASE ]; then
                cp $VSP_VMLOCAL_DHCPD_LEASE $VSP_VAR_VMLOCAL_DHCPD_LEASE_DIR
        ...
        ;;
    562_563)
        # recreate missing EX dir for force downgrade
        MFG_TYPE=`/opt/tms/bin/hald_model -m`
        if [ "x$MFG_TYPE" = "xrvbd_ex" ]; then
            mkdir -p /var/vmware
        fi
