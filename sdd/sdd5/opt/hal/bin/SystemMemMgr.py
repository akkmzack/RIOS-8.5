...
def get_total_mem():
    ...
    if not exists (hwtool_path):
        rlog_warning('Hwtool does not exist on this box. Exiting...')
        exit(1)
...
# cmd_get_kernel_mem
#
# Get memory which the kernel does not display in /proc/meminfo
# For simplicity its 0.02 * total memory
#
###############################################################################
def cmd_get_kernel_mem(option, spec, model):
    print int(round(float(get_total_mem() * 0.02)))
...
def get_vsp_memory(spec):
    ...
    # if no addon value defined, we default to minimum 
    (vsp_max, total_max) =  (vsp_min, total_min)
    ...
    if total_sys_mem >= total_max:
        ...
        # is equal to or more than the 'total_max' from specs, if so vsp memory
        # available is vsp_max. ADDON kit was added OR the customer added
        # more than the max supported system memory.
        rsp_mem = vsp_max
    elif total_sys_mem >= total_min:
        # Check to see if the total available memory in the system
        # is equal to or more than the 'total_min' from specs, if so vsp memory
        # available is vsp_min. NO ADDON KIT in the system OR the customer added
        # partial DIMM's from the addon kit
        rsp_mem = vsp_min
    else:
        # If this is the case (total_sys_mem < total_min), then there isn't enough
        # memory in the system to run vsp, set value to 0
        rsp_mem = 0
...
def get_edge_mem(spec):
    if get_appliance_uses_granite() == 'true':
	    ...
	    # There exists a memory rule. Parse this memory rule and compare it with total system memory
		    ...
		    rlog_warning('MFDB returned null for edge mem. Exiting...')
		    exit(1)
        else:
...
def cmd_get_edge_mem(option, spec, model):
	...
	rlog_notice('Edge/Granite is not supported on this model [%s]' % model)
...
cmd_handlers = { '--total-memory'     : cmd_get_total_mem,
		 ...
		 '--kernel-overhead'  : cmd_get_kernel_mem,
...
def setup_parser():
    ...
    parser.add_option ("--kernel-overhead",  action="callback", callback=sys_mem_mgr_cmd_callback,
                        dest="kernel-overhead", default=False, help="Amount of memory that the kernel does not even show in /proc/meminfo")
    ...
    model = get_model()
    if not model:
        rlog_warning('Could not determine model, exiting')
        exit(1)
    ...
    try:
        ...
        rlog_warning('No command handler registered for cmd %s' % cmd)
        exit (1)
