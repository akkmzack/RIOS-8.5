...
CACHED_MOBO_IS_VM=${HAL_CACHE}/motherboard_is_vm
...
get_motherboard_is_vm()
{
    if [ -f ${CACHED_MOBO_IS_VM} ]; then
        MOBO_IS_VM=`cat ${CACHED_MOBO_IS_VM}`
        if [ "x${MOBO_IS_VM}" != "x" ]; then
            echo ${MOBO_IS_VM}
            return
        fi
    fi

    MOBO_IS_VM=`${HWTOOL_PY} -q motherboard_is_vm`
    if [ $? != 0 ]; then
        ${HAL_LOG_WARN} "Hwtool unable to determine whether motherboard is virtual"
        exit 1
    fi

    # If we don't have a set up /var filesystem, we might not be able to create 
    # this directory. That's ok. If we can't create it just return the 
    # parameter, we'll try again next time.
    if [ ! -d ${HAL_CACHE} ]; then
        mkdir -m 0755 ${HAL_CACHE} > /dev/null 2>&1
    fi

    # only set the cache if we were able to make the cache dir.
    if [ -d ${HAL_CACHE} ]; then
        echo ${MOBO_IS_VM} > ${CACHED_MOBO_IS_VM}
    fi
...
check_kernel_options()
{
    ...
    KOPTS=`python -c 'import kernel; print kernel.get_kernel_cmdline()'`
    if [ $? -ne 0 ]; then
    ...
    if [ "x${KOPTS}" != "x" ]; then
        ...
        if [ $? -ne 0 ]; then
            ...
            echo "HAL: Need to reboot to get the kernel with the ${KOPTS} option"

            CUR_KOPTS=`/bin/cat /proc/cmdline`
            ${HAL_LOG_NOTICE} "Current kernel options are ${CUR_KOPTS}"
            echo "HAL: Current options are ${CUR_KOPTS}"
...
uses_system_led_control()
{
    ...
    case "x${MOBO}" in
	"x425-00140-01"|"x425-00205-01"|"x400-00100-01"|"x400-00300-01"|"x400-00099-01"|"x400-00300-10"|"x400-00098-01"|"x425-00120-01"|"x400-00400-01")
	    echo "true"
	;;
	*)
	    echo "false"
...
set_system_led_state()
{
    ...
    "x400-00400-01")
            case "x${STATE}" in
            "xnormal"|"xdegraded"|"xcritical")
                state=${STATE}    
            ;;
            *)
                ${HAL_LOG_WARN} "Invalid LED state requesed ${STATE}"
                return 1
            ;;
            esac
            output=`${HW_CTL_PY} -w led_strip alarm ${STATE}`
            error=$?
            if [ $error -ne 0 ] ; then
	...
	"x400-00100-01"|"x400-00300-01"|"x400-00300-10")
        ...
        "x425-00140-01"|"x425-00205-01")
            ...
            if [ -e /sys/class/system_led/led5/color ]; then 
                echo "$newstate" > /sys/class/system_led/led5/color 
                if [ $? -ne 0 ]; then
                    ${HAL_LOG_WARN} "Unable to set system led state to ${STATE}: error writing to sysfs file."
                    return 1
                fi
            else 
                ${HAL_LOG_WARN} "Unable to set system led state to ${STATE}: sysfs path not found."
                return 1    
            fi
        ;;
        # Guppy
...
get_system_led_color()
{
        ...
        MOBO=`get_motherboard`
        case "x${MOBO:0:9}" in
            "x425-00140"|"x425-00205"|"x400-00100"|"x400-00300"|"x425-00120"|"x400-00400")
                normal="Blue"
                degraded="Yellow"
                critical="Red"
                ;;
...
get_system_led_state()
{
    ...
    case "x${MOBO}" in
        ...
        "x400-00099-01"|"x400-00098-01")
            ...
            ;;
        "x400-00400-01")
            ${HW_CTL_PY} -r led_strip alarm normal | grep True > /dev/null
            if [ $? -eq 0 ]; then
                echo "normal"
                return 0
            fi
            ${HW_CTL_PY} -r led_strip alarm degraded | grep True > /dev/null
            if [ $? -eq 0 ]; then
                echo "degraded"
                return 0
            fi
            ${HW_CTL_PY} -r led_strip alarm critical | grep True > /dev/null
            if [ $? -eq 0 ]; then
                echo "critical"
                return 0
            fi
        ...
        "x425-00120-01")
            ${HW_CTL_PY} -r led_strip system ok    | grep True > /dev/null
            ok_no_alarm=$?
            ${HW_CTL_PY} -r led_strip system minor | grep True > /dev/null
            minor_alarm=$?
            ${HW_CTL_PY} -r led_strip system major | grep True > /dev/null
            major_alarm=$?
            ${HW_CTL_PY} -r led_strip system off   | grep True > /dev/null
            off_alarm=$?

            alarm_counts=$((ok_no_alarm+minor_alarm+major_alarm+off_alarm))
            if [ $alarm_counts -ne 3 ]; then
                # The HW_CTL_PY return code is 0 if return True, else 1.
                # So proper alarm_counts should count to 3 (1 True, 3 Falses).
                # Any other alarm_counts # indicates an invalid LED state.
                ${HAL_LOG_WARN} "Unknown LED status for system LED"
                echo "unknown"
                return 1
            fi
            if [ $major_alarm -eq 0 ]; then
                echo "critical"
                return 0
            fi
            if [ $minor_alarm -eq 0 ]; then
                echo "degraded"
                return 0
            fi
            if [ $ok_no_alarm -eq 0 ]; then
                echo "normal"
                return 0
            fi
            ;;
...
get_hw_if_status()
{
    ...
    PREFIX=`find_nic_target ${wan_lan}`
    if [ $? -ne 0 ]; then
	exit 1
    else
        ...
        MOBO=`get_motherboard`
        case "${MOBO:0:9}" in
            "425-00120"|"400-00098"|"400-00099")
                # Guppy and Minnows to retry, as read back result maybe wrong.
                # Most times the read back is correct.  But there is a 0.2%
                # failure rate, which is indicated by CODE equals to 1.  Retry
                # up to 5 times to lower the failure rate to negligible.
                retry_count=0
...
bp_util_supports_normal()
{
    utility=$1
    case "${utility}" in
        "${BYPASS_CTL}"|"${MINNOW_CTL}"|"${NA0043_CTL}"|"${ADLINK_CTL}"|"${HP_CTL}"|"${GUPPY_CTL}")
            echo "True"
        ;;
        *)
            echo "False"
...
bp_util_supports_bypass()
{
    ...
    case "${utility}" in
        ...
        "${BYPASS_CTL}"|"${MINNOW_CTL}"|"${NA0043_CTL}"|"${HP_CTL}"|"${GUPPY_CTL}")
            echo "True"
        ;;
        *)
            echo "False"
...
bp_util_supports_block()
{
    ...
    case "${utility}" in
        ...
        "${BYPASS_CTL}"|"${MINNOW_CTL}"|"${HP_CTL}"|"${GUPPY_CTL}")
            echo "True"
        ;;
        *)
            echo "False"
...
get_avail_speed_duplex()
{
	INTF="${1}"
        ...
        if [ "x${INTF}" = "xprimary" -a ! -e /sys/class/net/primary/device ]; then
            INTF="prihw"
        fi

        #If interface is aux, make sure that it is not a virtual interface
        if [ "x${INTF}" = "xaux" -a ! -e /sys/class/net/aux/device ]; then
            INTF="auxhw"
        fi
...
set_speed_duplex()
{
	...
	INTF="${1}"
        ...
        #If interface is aux, make sure that it is not a virtual interface
        if [ "x${INTF}" = "xaux" ]; then
           RET=`ls /sys/class/net/aux/device 2>/dev/null`
           if [ $? -eq 1 ]; then
                INTF="auxhw"
           fi
        fi
...
get_default_ipmi_wdt_timeout()
{
    ...
    case "${MOBO:0:9}" in
        "400-00300"|"400-00100"|"425-00205"|"425-00140"|"425-00220")
...
# Start specific RAID devices instead of all the remaining devices as is done in
# do_start_rios_volumes, the volume name will be passed as a parameter
#
# Logging is OK here since we would have started /var above.
#
do_start_specific_rios_volume()
{
    local VOLNAME=$1
    USES_SW_RAID=`${RRDM_TOOL} --uses-sw-raid`
    if [ "x${USES_SW_RAID}" = "xTrue" ]; then
        LINE=`${RRDM_TOOL} -l | /bin/grep ${VOLNAME}`
...
#
# given a start and end disk index, print a row of drive bays
# the existence of param 4 requests to print newline when done
#-------------------------------------------------------------------------------
draw_dual_drive_row()
{
        START="$1"
...
#-------------------------------------------------------------------------------
# draw_drive_slot_number <slot> <EOL>
#
# if there are no parameters passed then just draw enpty space.
#
# draw something similar to draw_drive_bay() except display the slot number.
# the status of the drives in that slot will be drawn beneath it.
# if $2 is present then print a newline
#-------------------------------------------------------------------------------
draw_drive_slot_number()
{
        ...
        if [ $# -eq 0 ]; then
            ...
            echo -n "[${PADDING:0:${PREPAD_LEN}} SLOT ${SLOT} ${PADDING:0:${POSTPAD_LEN}} ]"
        fi
...
###############################################################################
# find_blockdev_fstype
#
# Given a block device determine what type of FS/format is resident on the device
# this could be ext3/ext4, linux_raid_member, or blank if its data
###############################################################################
find_blockdev_fstype()
{
    BDEV="$1"
...
find_volume_fstype()
{
    ...
    VOLDEV=`${RRDM_TOOL} -l | grep ${VNAME} | cut -f2 -d:`

    if [ "x${VOLDEV}" != "x" ]; then
        FSTYPE=`find_blockdev_fstype /dev/${VOLDEV}`
        if [ "x${FSTYPE}" = "x" ]; then
            # translate no answer to raw data type
            echo "raw"
        else
            echo $FSTYPE
        fi
...
# (un)load_module
#
# load or unload a given kernel module
###############################################################################
