...
create_1050H_pfs_raid()
{
    /sbin/mdadm --create /dev/${PFS_DEV} --run \
        --level=linear --raid-devices=2 -e 0.90 \
...
hal_1050H_raid_prep()
{
    ...
    # XXX/munirb:
    # The reason we do this dance is cause we want the RAID superblock
    # on the segstore disks even though we don't use it, it was done 
    # initially cause we were unsure whether we will use RAID or not
    # now we are paying the price for backwards compatibility
    rm -f /dev/${SS_DEV}
    mknod /dev/${SS_DEV} b 9 0

    /sbin/mdadm --create /dev/${SS_DEV} --run \
        --level=raid0 --raid-devices=2 -e 0.90 \
    ...
    do
        ...
        # Check if the SS_DEV array is stopped if not retry
        if [ $? -ne 0 ]; then
            # Retry a mdadm stop 
            sleep 1
            echo "Retrying to stop [/dev/${SS_DEV}] array"
        else
            echo "Stopped [/dev/${SS_DEV}] array"
            break
        fi
    done

    # Create the PFS ready file so that we don't come here
    touch /var/opt/rbt/.samba_ready

    if [ $i -eq 5 ]; then
        # Reboot the box cause we were unable to stop the md0 RAID array and
        # that means sport wont start, reboot should release the resources
        /sbin/reboot
    fi
}
