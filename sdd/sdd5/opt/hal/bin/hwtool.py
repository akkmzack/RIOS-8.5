...
def main():
	...
	get_rev()
        ...
        print_address_map(gateway_naming)
    elif query == "cli":
	get_cli()
    elif query == "if_util":
	get_util(query, arg)
...
def get_numa(subops):
    if subops == "enabled":
        pipe = subprocess.Popen('ls -d /sys/devices/system/node/node[0-9]* | wc -l', 
                shell=True, stdout=subprocess.PIPE)
        num_nodes = int(pipe.communicate()[0].strip())
        if num_nodes > 1:
            print 'yes'
        else:
            print 'no'
