...
do
    message debug "Backing up tdb file $FILE ..."
    ...
    if [ $FILE_SIZE -gt $MAX_SIZE ]; then
        ...
        if [[ $FILE != "/var/samba/private/secrets.tdb"  && $FILE != "/var/samba/var/locks/winbindd_idmap.tdb" ]]; then
            message warn "Removing and regenerating $FILE"
            rm -f $FILE;
            CORRUPTED="TRUE";
        fi
        continue;
    ...
    COMMAND="$TDB_CMD $FILE"
    run_cmd_with_timeout "$COMMAND" "$CMD_TIMEOUT"
    EXIT_STATUS=$?
    if [ $EXIT_STATUS -eq 0 ]
    then
	message debug "tdbbackup for file $FILE successful"
    elif [ $EXIT_STATUS -eq 1 ]
    then
	message error "Error in doing tdbbackup file $FILE: Error = $EXIT_STATUS"
	message error "$FILE might be corrupt, moving it..."
