...
#
#  URL:       $URL: svn://svn/mgmt/branches/malta_529_fix_branch/framework/src/base_os/common/script_files/sysdump.sh $
...
# This is used to dump ipv6 related kernel data.
IPV6_RUNNING=0
if [ -n "`lsmod | grep -w ipv6`" ]; then
    IPV6_RUNNING=1
fi
...
show_ecc_status()
{
    for i in /sys/devices/system/edac/mc/mc[01]/*/{*_count,*_label,size_mb}; do
        echo -n $i:
        cat $i
    done
}
...
show_firmware_version()
{
    ...
    local MOBO=`hwtool -q motherboard | awk -F- '{ print $1"-"$2}'`
    echo "==================================================" >> ${FILENAME}
...
show_ipmi_info()
{
    local CMD_NAME="ipmitool sel list -v"
    local ERR_LOG="ipmitool sel list"
    local INFO_CMD_NAME="ipmitool mc info"
...
show_hardware_err_log()
{
    ...
    echo "" >> "${HARDWARE_INFO_FILENAME}"
    echo "${ERR_LOG_CMD}" | cli >> "${HARDWARE_INFO_FILENAME}"
    echo "" >> "${HARDWARE_INFO_FILENAME}"
    echo "==================================================" >> "${HARDWARE_INFO_FILENAME}"
}
...
show_smart_data_smartctl()
{
    ...
    local MOBO=`hwtool -q motherboard`
    case ${MOBO:0:9} in
        "400-00100"|"400-00300"|"425-00205"|"425-00140"|"400-00400")
...
do_command_output_file 'cat /var/opt/rbt/.unscheduled_reboots' "${HARDWARE_INFO_FILENAME}"
...
do_command_output 'netstat -a --numeric-hosts'
do_command_output 'arp -na'
if (($IPV6_RUNNING)); then
    do_command_output "ip -6 neigh show"
fi
do_command_output 'ps -Aww -o user,pid,ppid,pcpu,pri,nice,vsize,rss,majflt,tty,stat,wchan=WIDE-WCHAN-COLUMN -o start,bsdtime,command'
...
# HP blade specific graft.
if [ "$HAVE_HPBLADE_SYSDUMP_GRAFT" = "y" ]; then
...
for IF_NAME in ${IF_LIST}; do
    ...
    if [ "VM" != `hwtool -q motherboard | awk '{ print $1}'` ]; then
...
if [ -f ${XBRIDGE_XUTIL} ]; then
    ...
    do_command_output_file "${XBRIDGE_XUTIL} --print-route-status" "${STAGE_DIR}/xbridge_route_status"
...
if [ ${BRIEF} -eq 0 ]; then
    do_command_output_file "cat /proc/net/tcp" "${STAGE_DIR}/proc_net_tcp"
fi
...
#
# Copy over SSL servers global exportable status, 
# if we can...
#
