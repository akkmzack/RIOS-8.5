...
def get_prod_type():
    if not exists('/opt/hal/bin/hwtool.py'):
        raise HwtoolError("In manufacturing, cannot find product type")
...
def get_bios(query):
    if query == 'serialnum':
        ...
        try:
            ...
            print "Error reading from /dev/mem"
            exit(1)

        buf = buf.rstrip()[:-1]
        hash = buf[3:5] #OEM alert: will need to be [4:6] for oem units
        sport = buf[5:]
        val = str(md5.new(sport + 'make babies\n').hexdigest())
        comp_hash = dict[int(val[30], 16)] + dict[int(val[31], 16)]
        if hash == comp_hash:
            print buf
    elif query == "version":
        try:
...
def get_util(query, slot):
    ...
    nic = NIC(mobo)
    if mobo.isbob == 'true':
        nic.get_bob_nic_info(nics, slot, query)
    else:
        nic.get_nic_info(nics, slot, query)
...
def print_card_info(mobo, nic, links):
    partnum = mobo.part_num[0:9]
    for (n, s) in mobo.slot_patterns:
        l = nic.get_links_from_pattern(s, links)
        if l != []:
            card = nic.get_card_from_link(nics, cards, l[0])
            if card == None:
                dname = "Unrecognized card"
                part_num = "Unknown"
            else:
...
def get_address_map(gateway_naming, return_map, get_branding):
    mobo = get_mobo()
    nic = NIC(mobo)
    nic.get_address_map(nics, cards, gateway_naming = False, return_map = False, get_branding = False)


def print_nic_license():
    mobo = get_mobo()
    nic = NIC(mobo)
    map = nic.get_address_map(nics, cards, gateway_naming = False, return_map = False, get_branding = True)
    for iface in map:
        print "%s %s" % (iface[0], iface[1])
...
def print_licensed(pretty = True): 
    if pretty:
        ...
        mobo = get_mobo()
        nic = NIC(mobo)
        (map, nic_brand_state) = nic.get_address_map(nics, cards, gateway_naming = False, return_map = True, get_branding = False)
        if nic_brand_state:
            nic_brand = "false"
...
def get_mactab(out_path, gateway_naming):
    ...
    if mobo.isbob == 'true':
        nic = NIC(mobo)
        nic.get_bob_mactab(nics, out_path)
    else:
        nic = NIC(mobo)
        nic.get_mactab(nics, cards, out_path, gateway_naming)
...
def get_has_card(arg):
    mobo = get_mobo()
    nic = NIC(mobo)
    print nic.has_card(nics, cards, arg)
...
def get_card_list():
    ...
    nic = NIC(mobo)
    if mobo.isbob == 'true':
	nic.get_bob_card_list(nics, cards);
    else:
	nic.get_card_list(nics, cards)
...
def print_kernel_opts():
    ...
    if mobo.part_num == 'VM':
        arch = uname('-i')
        # XXX/munirb: Bug 102019
        # Get rid of the 'clock=pmtmr' default option for 64 bit VM's
        # its not supported
        kopts = 'notsc nohpet'
        if arch == 'i386':
            # For 32 bit machines append to the default clock=pmtmr option
            kopts = '%s hpet=disable' % mobo.kernel_opts

        try:
            prod = get_prod_type()[0]

            if prod == 'GW' or prod == 'CMC' or prod == 'DVA':
                print "%s divider=10" % kopts
            else:
                print kopts
        except HwtoolError:
