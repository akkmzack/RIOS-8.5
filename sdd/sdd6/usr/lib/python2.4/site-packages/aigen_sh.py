...
from os import system, popen, rmdir, mkdir
...
def generate_grub(next_boot):
    ...
    mkdir("/var/tmp/ai/")
    err = system("mount %s%d /var/tmp/ai/ > /dev/null 2>&1" %
                    (disk, oboot))
    if booted_img == 1:
        img1_initrd = exists('/boot/rbt-initrd.img')
        img2_initrd = exists('/var/tmp/ai/rbt-initrd.img')
    else:
        img1_initrd = exists('/var/tmp/ai/rbt-initrd.img')
    ...
    if booted_img == 1: 
        ...
        img2 = "/var/tmp/ai/etc/build_version.sh"
    else:
        img1 = "/var/tmp/ai/etc/build_version.sh"
    ...
    try:
        ...
        system("umount /var/tmp/ai/ > /dev/null 2>&1")
        rmdir("/var/tmp/ai/")
        print "Error:", x
        exit(1)
