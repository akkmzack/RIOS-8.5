...
class VolumeConfig:
    ...
    # volume. 
    # ["swap", "ext3", "none"] are currently supported.
    def get_fstype(self):
    ...
    def validate(self, layout):
        if self.get_type() not in [ "single", "raid10", "utility", "raid5", "raid6", "ftraid" ]:
        ...
        if self.get_type() in [ "raid10", "raid5", "raid6" ]:
            if self.get_rdev() == '': 
        ...
        if self.get_fstype() not in [ "none", "swap", "ext3" ]:
...
#
class Ext3FormatMethod(LogicalVolumeFormatMethod):
    def __init__(self,
                 fstype,
                 xml):
        if fstype != 'ext3':
            raise AssertionError('Invalid LVM fstype [%s] for Ext3FormatMethod' % fstype)
...
class FormatMethodFactory:
    ...
    def create_format_method(self,
        ...
        elif method == self.ext3_method:
            return Ext3FormatMethod(fstype, xml)
        else:
