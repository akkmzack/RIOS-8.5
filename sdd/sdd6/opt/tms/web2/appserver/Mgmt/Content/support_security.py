...
class gui_Security(PagePresentation):
    ...
    def setupSecurity_capabilityAccounts(self):
        ...
        if 'editCapabilityAccount' in self.fields:
            ...
            account = self.fields.get('editCapabilityAccount_account')
            ...
            if self.fields.get('editCapabilityAccount_usepw'):
                OSUtils.log(Logging.LOG_NOTICE, 'Changing password for %s' % account)
                password = self.fields.get('editCapabilityAccount_password')
                if password and (bogusPassword != password):
                    if self.isCMC():
                        if account == 'admin':
        ...
        if id:
            ...
            if cmc:
                ...
                obj = {'address': address,
                       ...
                       'key': key,
                       'enable': enable}
                self.editNodeSequence(base, radiusSpec, 'add', index, obj)
            else:    # SH
            ...
            if cmc:
                ...
                self.editNodeSequence(base, radiusSpec, 'edit', int(index), obj)
            else:    # SH
            ...
            if cmc:
                for id in ids:
                    self.editNodeSequence(base, radiusSpec, 'remove', id)
            else:  # SH
                for id in ids:
...
a string to set the key.
        ...
        if id:
            ...
            if cmc:
                ...
                obj = {'address': address,
                       ...
                       'key': key,
                       'enable': enable}
                self.editNodeSequence(base, tacacsSpec, 'add', index, obj)
            else:    # SH
            ...
            if cmc:
                ...
                self.editNodeSequence(base, tacacsSpec, 'edit', int(index), obj)
            else:    # SH
                params = [('server_idx', 'uint32', index),
                          ('address', 'hostname', address),
                          ('auth-port', 'uint16', authPort),
            ...
            if cmc:
                for id in ids:
                    self.editNodeSequence(base, tacacsSpec, 'remove', id)
            else:  # SH
                for id in ids:
    ...
    def setupSecurity_usersAction(self):
        ...
        if 'addRbmUser' in fields:
            ...
            if fields.get('addRbmUser_usepw'):
            ...
            setlist = [(prefix % 'enable', 'bool',
                        fields.get('editRbmUser_enable', 'false'))]
            if self.fields.get('editRbmUser_usepw'):
                password = self.fields.get('editRbmUser_password')
                if password and (bogusPassword != password):
                    if self.isCMC():
                        self.setNodes((prefix % 'password_sha512', 'string',
                                       common.sha_encrypt_password(False, password)))
                    setlist.append((prefix % 'password', 'string',
                                    self.encryptPassword(password)))
            else:
                setlist.append((prefix % 'password', 'string', ''))
            self.setNodes(*setlist)
            ...
            # Only remove those denied role nodes that were previously
            # assigned to the account.
            removedPermissions = [k for k, v in rolePermissions
                                  if v == 'deny'
                                  and k in userRoles]
            # Do the removal.
...
parentKey, address, auth-port, key, timeout, retransmit, status.
        ...
        for u in rbmUserList:
            ...
            self.xmlizeAttributes(userEl, users[u],
                                  ('enable',
                                   ...
                                   'shell',
                                   'uid'))
            password = users[u].get('password', '')
            userEl.setAttribute('password', password and bogusPassword)
    ...
    def capabilityAccounts(self):
        ...
        for user in ('admin', 'monitor'):
            ...
            password = Nodes.present(self.mgmt, base + '/' + user + '/password', '')
            el.setAttribute('password', password and bogusPassword)
    ...
    def localUser(self):
        ...
        sortedRoles = RBA.getSortedRoleSubset(roles.keys())
    ...
    def setupFirewall(self):
        ...
        def internal(responseEl, **fields):
            ...
            cmcPathPrefix = 'editPolicy' in fields \
                and Nodes.cmcDecodePolicy(fields['editPolicy'])[1] or None
            ruleList = FirewallRuleList(
                self.mgmt, cmcPathPrefix=cmcPathPrefix)
    ...
    def firewallRules(self):
        ...
        cmcPathPrefix = 'editPolicy' in self.fields \
            and Nodes.cmcDecodePolicy(self.fields['editPolicy'])[1] or None
        ...
        tablizeRules(
            FirewallRuleList(self.mgmt, cmcPathPrefix=cmcPathPrefix), True)
        table.close()
...
class FirewallRuleList(object):
    ...
    def __init__(
        ...
        # Sanity check: make sure that the last ord is equal to the
        # number of rules.  The default rules start counting at 0 but
        # the mutable ones start at 1.
        if self.__defaultRules:
            assert ords == [] or int(ords[-1]) == (len(ords) - 1), \
                'wrong number of rules'
        else:
            assert ords == [] or int(ords[-1]) == len(ords), \
                'wrong number of rules'
    ...
    def __rewriteCmcPolicyNodes(self):
        ...
        for o in Nodes.getMgmtLocalChildrenNames(
                self.__mgmt, self.__aclRulePrefix):
            Nodes.delete(self.__mgmt, '%s/%s' % (self.__aclRulePrefix, o))
        for i in range(len(self.__rules)):
            rulePrefix = '%s/%i' % (self.__aclRulePrefix, i + 1)
            nodes = self.__rules[i].getConfigParameters(rulePrefix)
