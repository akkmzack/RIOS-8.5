...
do_update_model_params()
{
    ...
    # Since the VSH model class is "Virtual" we cannot strip out the
    # characters from the class. Hence we check for a VSH with this work-around
    MOBO=`get_motherboard`
    PLAT=`get_platform`

    if [ "x${PLAT}" = "xSH" -a "x${MOBO}" = "xVM" ]; then
        BASE_MODEL="Virtual"
    else 
        BASE_MODEL=`echo "${TARGET_MODEL}" | tr -d "a-zA-Z_"`
    fi

    do_update_mfdb /rbt/mfd/model string "${BASE_MODEL}"
