...
def list_volumes(system):
    ...
    for arr in system.raid_arrays.get_array_list(): 
        ...
        # to reserve space.
        ret += '%s:%s:%s:%s:false:true\n' % (arr.name, arr.dev_name, arr.fstype, arr.type)
    ...
    for arr in system.volumes: 
        ...
        ret += '%s:%s:%s:%s:%s:false\n' % (arr.name, arr.dev_name, arr.fstype, arr.type, reserve_sb_space)
...
def cmd_get_raid_configuration(option, spec, model):
    ...
    try:
        ...
        order = scdm.get_disk_config(profile_name).get_zone_map()['mgmt'].get_disk_index_list()
    except:
    ...
    #
    for array in system.raid_arrays.get_array_list():
	array.display_configuration(order)
...
def cmd_list_mount_points(option, spec, model):
    ...
    for lvm in lvm_list:
        if lvm.get_mount() not in [ '', None ]:
            print '%s:%s:%s' % (lvm.get_name(), lvm.get_fstype(), lvm.get_mount())
    ...
    for lvm in lvm_list:
        ...
        if lvm.get_mount() not in [ '', None ]:
            ...
            if do_mount and devname != None:
                err = system('/bin/mount /dev/%s %s' % (devname, 
                                                        lvm.get_mount()))
                if err:
                    errmsg = 'failed to mount device on %s on %s' % \
                              (devname, lvm.get_mount())
                    print errmsg
                    rlog_warning(errmsg)
                else:
                    print 'Device %s mounted on %s' % (devname,
                                                       lvm.get_mount())
...
# cmd_get_list
#
# displays the volume list used for setup and manufacturing in colon seperated 
# form
#
###############################################################################
...
###############################################################################
# rrdm_spare_sfdisk_str
#
# Generate the sfdisk string using the DiskLayout passed in.
# 
# returns the sfdisk string; None on failure
#
###############################################################################
def rrdm_spare_sfdisk_str(dl):
    if dl == None:
...
#
###############################################################################
def rrdm_spare_partition_drive(drive_number, sfdisk_str, dry = True):
    try:
        name="/tmp/spare_part"
	tfile = open(name, "w+b")
	part_str = sfdisk_str
...
def cmd_fmt_spares(option, spec, model, mfg_mode = True):
    ...
    # Generate the sfdisk layout 
    sfdisk_str = rrdm_spare_sfdisk_str(dl)
    if sfdisk_str == None or len(sfdisk_str) == 0:
	...
	for drive in spare_list:
	    ...
	    rrdm_spare_partition_drive (drive, sfdisk_str, False)
	    # Clear labels 
