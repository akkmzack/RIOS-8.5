#
#  Filename:  $Source$
#  Revision:  $Revision: 122844 $
#  Date:      $Date: 2013-03-07 19:37:10 -0800 (Thu, 07 Mar 2013) $
#  Author:    $Author: pipatel $
# 
...
writeimage_graft_4()
{
    # check to make sure we are installing the right kind of image
    ...
    if [ -f /config/mfg/mfdb -a -x /opt/tms/bin/mddbreq ]; then
        ...
        # Limit this check to SH only. EX will have different release version numbers (e.g. 2.x.x)
        # This check will break EX upgrades if it shares CPU with CX (e.g EX1360L)
        if [ "x$BUILD_PROD_ID" = "xSH" ]; then
	    # Check if the upgraded image version supports this CPU type
            # Releases <= 7.0.2 do not support the AMD valencia processor 42XX
	    CPU_VENDOR_ID=`cat /proc/cpuinfo | grep vendor_id | head -1 | cut -d':' -f2 | tr -d ' '`
	    if [ "x${CPU_VENDOR_ID}" = "xAuthenticAMD" ]; then
	        # Check the processor type now
	        CPU_SERIES=`cat /proc/cpuinfo | grep "model name" | head -1 | cut -d':' -f2 | tr -d ' ' | awk '{print substr($0,length($0)-3,4)}' | cut -b 1-2`
	        if [ "x${CPU_SERIES}" = "x42" ]; then
		    # Check the product release ID
		    SUB_VERSION_NUMBER_FIRST_CHAR=`echo ${SUB_VERSION_NUMBER} | cut -d '-' -f1 | tr -d [a-zA-Z]`
		    if [ $MAJOR_VERSION_NUMBER -eq 7 ]; then
	    	        if [ $MINOR_VERSION_NUMBER -eq 0 -a $SUB_VERSION_NUMBER_FIRST_CHAR -le 2 ]; then
			    # Do not allow upgrade
			    echo "ERR: Cannot install the upgrade image as it is older than the originally manufactured image."
			    exit 22 #lc_err_upgrade_cpu_not_supported
		        fi
		    fi

		    if [ $MAJOR_VERSION_NUMBER -lt 7 ]; then
		        # Do not allow upgrade
                        echo "ERR: Cannot install the upgrade image as it is older than the originally manufactured image."
                        exit 22 #lc_err_upgrade_cpu_not_supported
		    fi
...
writeimage_graft_5()
{
    ...
    echo "${curr_target_dev}" | grep "md" > /dev/null 2>&1
    if [ $? = 0 ] && [ "x$IL_LAYOUT" == "xFLASHRRDM_RAID" ]; then
	...
	echo "(hd0) /dev/${hd0}" >> ${mount_point}/boot/grub/device.map
	echo "(hd1) /dev/${hd1}" >> ${mount_point}/boot/grub/device.map
	REWRITE_DM=1
    fi
